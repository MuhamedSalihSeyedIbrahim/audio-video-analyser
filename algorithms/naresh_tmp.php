 ...
 */
 ...
 //Obtains two json data and provides the comparison and send slike terms and their percentage
    ...
    $brand1=$faces["summarizedInsights"]["brands"];
    $faces=$faces["summarizedInsights"]["faces"];
    foreach($faces as $current) {
      if($current["title"]!=null)
          $data1[]=$current["title"];
      }
    $faces=json_decode($json2,true)
    
    ["summarizedInsights"]["faces"];
    $brand2=$faces["summarizedInsights"]["brands"];
    foreach($faces as $current) {
      if($current["title"]!=null)
        $data2[]=$current["title"];
    }
    ...
    foreach($brand1 as $key=>$current) 
      if($current["name"]!=null)
        $data1[]=$current["name"];
    
    foreach($brand2 as $current) 
      if($current["name"]!=null)
        $data2[]=$current["name"];
        
    $matched["brands"]=array_intersect($data1,$data2);
    $match_percent["brands"]=(count($matched["brands"])/(min(count($data1),count($data2))+0.00001))*100;

...
"views": 0
     ...
     function state_identifier($json, $file_name) {
		...
		foreach($lines as $line) {
			...
			foreach($cur_line as $val)
			    $search_array[]=ucwords($val);
		}
		
        
        $full_data=fopen("/home/trendsep/public_html/sih2018/algorithms/datasets/state_database.txt","r");
        
        $cur_state=null;
        $full_state=array();
        
        while (!feof($full_data)) {
          $val=trim(fgets($full_data));
            if($cur_state=="*") {
                $cur_state=$val;
                $full_state[$cur_state]=array();
                continue;
            }
            if($val=="*") {
                $cur_state="*";
            ...
            $full_state[$cur_state][]=$val;
            if(in_array($val,$search_array)) {
                break;
            }
          
        }
        if ($val=="Tamluk" and !(in_array($val,$search_array)))
            return "Unknown";
        return $cur_state;
...
print_r(state_identifier($json,"jaipur_news.vtt"));
